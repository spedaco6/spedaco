...
describe("Validator class", () => {
  describe("Static methods", () => {
    describe("trimAndDespace", () => {
      test("trims values", () => {
        const value = Validator.trimAndDespace("   name    ");
        expect(value).toEqual("name");
      });
      test("replaces spaces with dashes", () => {
        const value = Validator.trimAndDespace("   test name   ");
    ...
    describe("removeAsterisk", () => {
      test("removes trailing asterisk from returned name", () => {
        expect(Validator.removeAsterisk("test*")).toHaveProperty("name", "test");
      });
      ...
      test("returns isRequired to true if trailing asterisk is present", () => {
        expect(Validator.removeAsterisk("test*")).toHaveProperty("isRequired", true);
      });
      test("returns isRequired to false if trailing asterisk is missing", () => {
        expect(Validator.removeAsterisk("test")).toHaveProperty("isRequired", false);
      });
    });
    ...
    describe("getAllValidators", () => {
      test("is defined in Validator class", () => {
        expect(Validator.getAllValidators).toBeDefined();
      });
      test("accepts a simple object", () => {
        const testInput = { name1: "value1", name2: 123, name3: true };
        const validators = Validator.getAllValidators(testInput);
        Object.entries(validators).forEach(([key, val]) => {          
          expect(Object.keys(testInput).includes(key)).toBe(true);
          expect(val instanceof Validator).toBe(true);
        });
      });
      test("accepts a FormData object", () => {
        const testInput = new FormData();
        testInput.append("name1", "value1"); 
        testInput.append("name2", "value2"); 
        testInput.append("name3", "value3"); 
        const validators = Validator.getAllValidators(testInput);
        Object.entries(validators).forEach(([key, val]) => {
          expect(Object.keys(testInput).includes(key)).toBe(true);
          expect(val instanceof Validator).toBe(true);
      ...
      test("creates any inputs listed in second argument array that are missing from the first argument object", () => {
        const testInput = { name1: "value1", name2: 123 };
        const includeInputs = ["name1", "name2", "name3"];
        const validators = Validator.getAllValidators(testInput, includeInputs);
        includeInputs.forEach(input => {
          expect(validators[input]).toBeDefined();
        });
      });
      test("requires all inputs listed in second argument array with trailing asterisk", () => {
        const testInput = { name1: "value1", name2: 123, name3: "" };
        const validators = Validator.getAllValidators(testInput, ["name1", "name2", "name3*"]);
        expect(validators.name3.errors).toHaveLength(1);
      });
      test("creates and validates all missing required inputs indicated by trailing asterisk*", () => {
        const testInput = { name1: "value1", name2: 123 };
        const validators = Validator.getAllValidators(testInput, ["name3*"]);
        expect(validators.name3).toBeDefined();
        expect(validators.name3.errors).toHaveLength(1);
      });
    });
    describe("getAllValues method", () => {
      test("is defined", () => {
        expect(Validator.getAllValues).toBeDefined();
      });
      test("accepts validators object", () => {
        const testInput = { name1: "value1", name2: 123, name3: false, name4: null, name5: undefined };
        const validators = Validator.getAllValidators(testInput);
        const testInput2 = Validator.getAllValues(validators);
        expect(testInput2).toBeDefined();
        expect(Object.entries(testInput2)).toHaveLength(5);
      });
      test("returns object containing keys of validators argument", () => {
        const testInput = { name1: "value1", name2: 123, name3: false, name4: null, name5: undefined };
        const validators = Validator.getAllValidators(testInput);
        const testInput2 = Validator.getAllValues(validators);
        const keys1 = Object.keys(testInput);
        const keys2 = Object.keys(testInput2);
        keys1.forEach(key => expect(keys2.includes(key)).toBe(true));
        keys2.forEach(key => expect(keys1.includes(key)).toBe(true));
        expect(keys1.length === keys2.length).toBe(true);
      });
      test("returns object containing values that correspond to each validators property", () => {
        const testInput = { name1: "value1", name2: 123, name3: false, name4: null, name5: undefined };
        const validators = Validator.getAllValidators(testInput);
        const testInput2 = Validator.getAllValues(validators);
        const values1 = Object.values(testInput);
        const values2 = Object.values(testInput2);
        values1.forEach(value => expect(values2.includes(value)).toBe(true));
        values2.forEach(value => expect(values1.includes(value)).toBe(true));
        expect(values1.length === values2.length).toBe(true);
      });
    });
    describe("getAllValidity method", () => {
      test("is defined", () => {
        expect(Validator.getAllValidity).toBeDefined();
      });
      test("accepts Validators argument", () => {
        const testInputs = { name1: new Validator("name1", "value1"), name2: new Validator("name2", "value2") };
        const result = Validator.getAllValidity(testInputs);
        expect(result).toBeDefined();
      });
      test("returns object with isValid property", () => {
        const testInputs = { name1: new Validator("name1", "value1"), name2: new Validator("name2", "value2") };
        const result = Validator.getAllValidity(testInputs);
        expect(result).toHaveProperty("isValid");
        expect(result.isValid).toBe(true);
        testInputs.name1.matches(testInputs.name2);
        const result2 = Validator.getAllValidity(testInputs);
        expect(result2).toHaveProperty("isValid");
        expect(result2.isValid).toBe(false);
      });
      test("returns object with validationErrors property", () => {
        const testInputs = { name1: new Validator("name1", "value1"), name2: new Validator("name2", "value2") };
        const result = Validator.getAllValidity(testInputs);
        expect(result).toHaveProperty("validationErrors");
        expect(Object.keys(result.validationErrors)).toHaveLength(0);
        testInputs.name1.matches(testInputs.name2).min(8);
        const result2 = Validator.getAllValidity(testInputs);
        expect(result2).toHaveProperty("validationErrors");
        Object.values(result2.validationErrors).forEach(val => expect(val).toHaveLength(2));
        expect(Object.values(result2.validationErrors).every(e => e.every(message => typeof message === "string"))).toBe(true);
        expect(Object.values(result2.validationErrors).some(e => e.some(message => message.includes("match")))).toBe(true);
      });
    });
    describe("required method", () => {
      ...
      test("String, number, boolean true, 0, and negative values return as valid", () => {
        const validValues = ["string", 823, true, "12", 0, -12];
    ...
    describe("matches method", () => {
      ...
      test("Accepts instance of Validator", () => {
        const instance = new Validator("name", "value");
        const { isValid } = Validator.matches(instance)("value");
        expect(isValid).toBe(true);
      });
      test("Returns Validator instance name with leading whitespace in error message", () => {
        const instance = new Validator("testName", "value");
        const { isValid, errors } = Validator.matches(instance)("");
    ...
    describe("isPassword method", () => {
      ...
      test.each(testPasswords)("Password requirements can be defined in configuration object", (password) => {
        expect(Validator.isPassword({ min: 5, lower: false, upper: false, special: false, number: false, max: 10 })(password).isValid).toBe(true);
        expect(Validator.isPassword({ max: 8 })("P@ssword1").isValid).toBe(false);
      });
    });
  ...
  describe("Instance tests", () => {
    ...
    describe("required method", () => {
      ...
      test("Empty strings, boolean false, null, and undefined return as invalid", () => {
        const invalidValues = ["", null, false, undefined];
        invalidValues.forEach(val => {
          const instance = new Validator("name", val);
      ...
      test("required message only returns when no other errors exist", () => {
        const instance = new Validator("test", "");
        const instance2 = new Validator("test2", "value");
        instance.required().matches(instance2);
        expect(instance.errors).toHaveLength(1);
        expect(instance.errors[0].includes("match")).toBe(true);
      });
    });
    describe("min method", () => {
      test("min method exists", () => {
    ...
    describe("matches method", () => {
      ...
      test("accepts a Validator instance as an input", () => {
        const instance = new Validator("testName", "test");
        const instance2 = new Validator("testName", "test");
        expect(instance.matches(instance2).isValid).toBe(true);
      });
    });
