...
describe("Validator class", () => {
  describe("Static methods", () => {
    ...
    describe("getAllValidators", () => {
      ...
      test("validates all required inputs", () => {
        ...
        Object.keys(validators).forEach(key => {
          expect(validators[key].errors).toHaveLength(key === "name3" ? 1 : 0);
        });
      });
      test("creates and validates all missing required inputs", () => {
        ...
        Object.keys(testInput).forEach(key => {
          expect(validators[key]).toBeDefined();
          expect(validators[key].errors).toHaveLength(key === "name3" ? 1 : 0);
        });
      });
    });
    describe("getAllValidity method", () => {
      ...
      test("returns object with errors property", () => {
        const testInputs = { name1: new Validator("name1", "value1"), name2: new Validator("name2", "value2") };
        const result = Validator.getAllValidity(testInputs);
        ...
        Object.values(result2.errors).forEach(val => expect(val).toHaveLength(2));
        expect(Object.values(result2.errors).every(e => e.every(message => typeof message === "string"))).toBe(true);
        expect(Object.values(result2.errors).some(e => e.some(message => message.includes("match")))).toBe(true);
      });
    });
