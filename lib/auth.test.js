...
vi.mock("@/lib/tokens", () => ({
  createAccessToken: vi.fn(() => "accessToken"),
  createRefreshToken: vi.fn(() => "refreshToken"),
}));
...
describe("auth actions", () => {
  describe("authenticateUser", () => {
    ...
    test("calls createAccessToken once with user object", async () => {
      await authenticateUser(email, password);
      expect(createAccessToken).toHaveBeenCalledOnce();
    });
    test("calls createRefreshToken once with user object", async () => {
      await authenticateUser(email, password);
