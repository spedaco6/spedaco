...
describe("auth actions", () => {
  describe("authenticateUser", () => {
    ...
    test("calls encode twice for access token and refresh token", async () => {
      await authenticateUser(email, password);
      expect(encode).toHaveBeenCalledTimes(2);
      const calls = encode.mock.calls;
    ...
    test("Creates new UserSession and saves to database", async () => {
      ...
      expect(refreshToken).toBe("encodedToken");
    });
    ...
    test("returns success object with access token and refresh token", async () => {
      ...
      expect(result).toHaveProperty("accessToken", "encodedToken");
      expect(result).toHaveProperty("refreshToken", "encodedToken");
    });
  });
});