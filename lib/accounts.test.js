...
import { createEmailVerificationToken, verifyPasswordResetToken } from "./tokens";
...
vi.mock("@/lib/tokens", () => ({
  createPasswordResetToken: vi.fn(() => "passwordResetToken"),
  createEmailVerificationToken: vi.fn(() => "verificationToken"),
...
describe("Account actions", () => {
  ...
  describe("createAccount", () => {
    ...
    test("calls createEmailVerificationToken once", async () => {
      User.findOne.mockImplementationOnce(() => Promise.resolve(null));
      await createAccount(formData);
  ...
  describe("Forgot Password", () => {
    ...
    test("adds passwordResetToken to user", async () => {
      ...
      const user = await mockSave.mock.results[0].value;
      expect(user).toHaveProperty("passwordResetToken", "passwordResetToken");
    });
    ...
    test("sendPasswordResetEmail called with user email and token", async () => {
      ...
      expect(email).toBe("email@email.com");
      expect(token).toBe("passwordResetToken");
  ...
  describe("completePasswordReset", () => {
    ...
    test("Invalid token returns success false with error", async () => {
      verifyPasswordResetToken.mockImplementationOnce(() => ({ error: "Error message" }));
